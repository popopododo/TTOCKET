...
function TicketHome() {
    ...
    <div className=''>
        ...
        {ticketList !== undefined && ticketSize !== undefined && ticketSize !== 0 &&
            <div className='flex overflow-x-auto snap-mandatory TicketList'>
                {ticketList.map((ticket : any, index : number) => 
                index < ticketSize ? <TicketHomeCard key={index} cardData={ticket} index={index}></TicketHomeCard> : null
                )}
            </div>
        }
        {ticketSize !== undefined && ticketSize === 0 &&  
            <div className='flex justify-center mt-32 place-items-center'>
                <div>
                    ...
                    <p className='mt-4 text-2xl font-bold text-center'>이용 가능한 티켓이 없습니다.</p>
                    <p className='mt-4 text-center text-gray-400'>티켓을 예매하고 티켓을 관리해보세요!</p>
                </div>
            </div>
        }
    </div>
